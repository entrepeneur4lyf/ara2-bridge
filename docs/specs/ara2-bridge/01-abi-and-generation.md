...
The canonical input is the official `https://github.com/Celemony/ARA_SDK.git` repository, never `reference/`. `scripts/install-ara-sdk.sh` installs top-level commit `a2b1aac1d1d5c4eed387db85a9c0cdb7d460254c` into the consuming project's ignored `.third-party/ARA_SDK/`, initializes pinned recursive submodules, builds the available examples, and records `ARA_SDK_DIR` in project-local Cargo configuration. Maintainers may provision the same identity through the component bootstrap. `.third-party/ARA_SDK/ARA_API/` commit `65ec5c43b943a48cb5446f448a0492db6af8534b` (`releases/2.3.0`) is the canonical ABI input. The top-level commit is one README-only commit after the release tag; it is recorded but does not alter normative code. The repository records every top-level/submodule commit, Git tree identity, license decision, and SHA-256 of every consumed or behavior-derived source. Regeneration fails if the GitHub checkout is absent, dirty, at the wrong identity, or hashes differ unless the explicit SDK-update workflow is invoked.
...
- 2026-07-16: Make the locked Celemony GitHub checkout under project-local `.third-party/` the sole SDK source; `reference/` is never an input, and Cargo discovers the checkout through installer-generated `ARA_SDK_DIR` configuration.
