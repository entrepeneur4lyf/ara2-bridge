...
Companion features are independent and off by default in the companion crate. `clap`, `vst3`, and `audio-unit-v2` enable only their required external dependencies. CLAP declarations are generated directly from CLAP tag `1.1.9`, commit `094bb76c85366a13cc6c49292226d8608d6ae50c`; no Rust CLAP sys crate is used. A project-local installer clones the official ARA SDK recursively plus VST3 `v3.8.0_build_66`, CLAP 1.1.9, and AudioUnitSDK 1.0.0, builds the available upstream examples, and records their paths in the consuming project's `.cargo/config.toml`. Cargo builds only consume those paths and never download code. The checked-in provenance manifests record exact commits, repositories, trees, and transitive input hashes.
