...
| Crate | Purpose |
|-------|---------|
| [`ara-bridge-sys`](ara-bridge-sys/) | Raw FFI bindings generated by `bindgen` from `ARAInterface.h`. 2,557 lines, 210 types, 55-field vtable structs. |
| [`ara-bridge`](ara-bridge/) | Safe Rust wrapper. `DocumentController` trait + vtable builder. Implements 25 of 55 vtable entries; null entries signal "not supported" per the ARA2 spec. |
...
This pulls in `ara-bridge-sys` automatically. You do not need to depend on it directly unless you want raw FFI access.
...
The `ARAFactory` struct is defined in `ara-bridge-sys`. Fill in the static fields:
...
ARA2 plugins are distributed as VST3 extensions. The `ARAVST3.h`, `ARACLAP.h`, and `ARAAudioUnit.h` headers in `ara-bridge-sys` provide the companion API types. You'll need:
...
Requires `clang` (for `bindgen`). The `ara-bridge-sys` crate generates bindings from `ARAInterface.h` at build time.
