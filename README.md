...
| Crate | Purpose |
|-------|---------|
| [`ara2-sys`](ara2-sys/) | Raw FFI bindings generated by `bindgen` from `ARAInterface.h`. 2,557 lines, 210 types, 55-field vtable structs. |
| [`ara2`](ara2/) | Safe Rust wrapper. `DocumentController` trait + vtable builder. Implements 25 of 55 vtable entries; null entries signal "not supported" per the ARA2 spec. |
...
This pulls in `ara2-sys` automatically. You do not need to depend on it directly unless you want raw FFI access.
...
```rust
use ara2::build_document_controller_instance;
...
The `ARAFactory` struct is defined in `ara2-sys`. Fill in the static fields:
...
ARA2 plugins are distributed as VST3 extensions. The `ARAVST3.h`, `ARACLAP.h`, and `ARAAudioUnit.h` headers in `ara2-sys` provide the companion API types. You'll need:
...
Requires `clang` (for `bindgen`). The `ara2-sys` crate generates bindings from `ARAInterface.h` at build time.
