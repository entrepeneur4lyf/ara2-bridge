...
sdk_environment = ["project-local .cargo/config.toml generated by scripts/install-ara-sdk.sh"]
...
platform_steps = ["CLAP and VST3 builds run on Linux, Windows, and macOS; Audio Unit v2 builds only on macOS", "VST3 v3.8.0_build_66 is locked under MIT"]
