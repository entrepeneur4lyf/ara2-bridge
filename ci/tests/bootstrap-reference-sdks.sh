...
git -C "$repo_root" check-ignore -q reference/ARA_SDK
...
mkdir -p "$empty_root/reference"
git clone --quiet --shared --no-checkout "$repo_root/reference/ARA_SDK" \
    "$empty_root/reference/ARA_SDK"
git -C "$empty_root/reference/ARA_SDK" remote set-url origin \
    https://github.com/Celemony/ARA_SDK.git
git -C "$empty_root/reference/ARA_SDK" checkout --quiet --detach \
    a2b1aac1d1d5c4eed387db85a9c0cdb7d460254c
printf '\ndirty test\n' >> "$empty_root/reference/ARA_SDK/README.md"
